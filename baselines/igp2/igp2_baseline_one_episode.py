...
def run_experiment(cost_factors: Dict[str, float] = None, use_priors: bool = False, max_workers: int = None):
    ...
    episode = scenario.load_episode(episode_id)

    # Scenario specific parameters
    SwitchLane.TARGET_SWITCH_LENGTH = scenario_config.target_switch_length
    ChangeLane.CHECK_ONCOMING = scenario_config.check_oncoming
    ip.Trajectory.VELOCITY_STOP = 1.
    Maneuver.NORM_WIDTH_ACCEPTABLE = 0.5
    Maneuver.LON_SWERVE_DISTANCE = 8

    if cost_factors is None:
        cost_factors = scenario_config.cost_factors

    test_data = [read_and_process_data(scenario_name, episode_id)]

    goals_data = scenario_config.goals
    if use_priors:
        goals_priors = scenario_config.goals_priors
    else:
        goals_priors = None
    ...
    with open(get_occlusions_dir() + f"{scenario_name}_e{episode_id}.p", 'rb') as file:
    ...
    logger.info(
        f"Starting experiment in scenario: {scenario_name}, episode_id: {episode_id},"
    ...
    grouped_data = test_data[0].groupby(['agent_id', 'ego_agent_id'])
    args = []
...
if __name__ == '__main__':
    ...
    experiment_name = scenario_name + f'_e{episode_id}' + config['output']
